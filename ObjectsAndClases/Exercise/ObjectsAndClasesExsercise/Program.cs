...
namespace ObjectsAndClasesExsercise
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                int p = ph.Next(0, phrases.Length);

                Random ev = new Random();
                int e= ev.Next(0, events.Length);

                Random au = new Random();
                int a = au.Next(0, authors.Length);

                Random ci = new Random();
                int c = ci.Next(0, cities.Length);
